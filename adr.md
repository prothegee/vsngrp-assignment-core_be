...
## ADR 021: migrations apply automatically on every deploy, not as a manual step

Decision: `cd.yml` now runs `dotnet ef database update --project src` right after `./containers.sh up`, inside a throwaway `mcr.microsoft.com/dotnet/sdk:10.0` container attached to `vsngrp-core-be_default`, using `CORE_BE_SED_CONFIG_PG_WRITE` directly as `MIGRATIONS_CONNECTION_STRING` (already the exact format `AppDbContextFactory` expects). The container has no NuGet cache, so `dotnet restore src` runs first, confirmed locally against a throwaway Postgres container before relying on it in production: without the restore step, `dotnet ef` fails with `NETSDK1064`, missing packages, not a real error in the migrations themselves.

Reason: a full datastore wipe (recovering from the stale-EC2 incident, see the project's own memory) also wipes the schema, migrations were previously a manual, undocumented-in-`cd.yml` step (`deployment.md` step 4), which is exactly how signup broke again immediately after that recovery, `500` on `/auth/signup`, `relation "accounts" does not exist`. `dotnet ef database update` is idempotent, already-applied migrations are skipped, so running it unconditionally on every deploy is safe and matches the same self-heal philosophy already used for the datastore containers and `config.json`.
