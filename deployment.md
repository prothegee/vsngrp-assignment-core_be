...
The `CORE_BE_SED_*` secrets are only read once, when `CORE_BE_CONFIG_PATH` does not exist yet and `cd.yml` creates it from the template. To rotate a value later (a leaked key, a changed database password), edit `config.json` directly on the instance, or delete it and let the next deploy recreate and reseed it from the current secrets.
...
3. Merging into `main-stable` triggers `cd.yml`, which connects over SSH and:
   ...
   - if `CORE_BE_CONFIG_PATH` does not exist yet, creates it from `config/config.json.template` and seeds `jwtSecret`, `postgres.write`, `postgres.read`, `redis`, and `corsAllowedOrigins` from the `CORE_BE_SED_*` secrets
...
The config file is never baked into the image. It is mounted read-only from the path in `CORE_BE_CONFIG_PATH` at container start. The first deploy creates and seeds it automatically, every deploy after that just reuses the existing file, `cd.yml` never overwrites a `config.json` that already exists.
